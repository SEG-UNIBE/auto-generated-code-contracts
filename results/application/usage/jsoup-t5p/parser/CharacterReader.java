...
//@ ensures bufSplitPoint == 0;
     ...
     */
//@ requires bufPos > 0;
//@ requires readerPos >= 0;
//@ ensures \result == readerPos + bufPos;
    public int pos() {
        return readerPos + bufPos;
    }

    /** Tests if the buffer has been fully read. */
//@ ensures \result == readFully;
    boolean readFully() {
        return readFully;
    }
...
//@ requires bufferUp!= null;
    ...
    /**
     ...
     @since 1.14.3
     */
//@ ensures \result == newlinePositions;
...
//@ requires isTrackNewlines();
//@ ensures \result == lineNumberOffset;
...
//@ requires readerPos >= 0;
//@ requires lineNumberOffset > 0;
//@ requires \nonnullelements(charBuf);
...
//@ requires charBuf!= null;
//@ requires bufPos > 0;
//@ ensures \result == charBuf[bufPos];
    char consume() {
        bufferUp();
...
//@ requires bufLength > 0;
//@ requires bufMark >= 0;
//@ ensures bufMark == bufPos;
    void mark() {
...
//@ requires bufPos >= 0;
//@ requires bufLength >= 0;
//@ ensures \result >= 0 && \result < charBuf.length;
...
//@ requires charBuf!= null && stringCache!= null;
    String consumeData() {
        // &, <, null
...
//@ requires charBuf!= null;
//@ requires bufPos >= 0;
//@ ensures \result == (seq.length >= 0);
    boolean matchesAnySorted(char[] seq) {
        bufferUp();
        return !isEmpty() && Arrays.binarySearch(seq, charBuf[bufPos]) >= 0;
    }

//@ requires charBuf.length > bufPos;
//@ ensures \result == (charBuf[bufPos] >= 'A' && charBuf[bufPos] <= 'Z');
...
//@ requires charBuf.length > bufPos;
//@ ensures \result == (charBuf[bufPos] >= '0' && charBuf[bufPos] <= '9');
...
//@ requires seq!= null;
//@ ensures bufPos == \old(bufPos) + seq.length();
...
//@ requires charBuf!= null && bufPos >= 0 && bufLength >= 0;
    @Override
...
//@ ensures \result!= null;
     ...
     */
//@ requires charBuf!= null && start >= 0 && start < charBuf.length;
//@ requires count >= 0 && count < cached.length();
...
//@ requires charBuf!= null && start >= 0 && start <= charBuf.length;
//@ requires count >= 0 && count <= charBuf.length;
//@ ensures \result == rangeEquals(charBuf, start, count, cached);
